...
The queued message starts the resumed session's first turn -- no prompt
needed, symmetric with the wake in step 2. (Prefer to resume quietly and
browse? `PI_POST_RESUME=stage` holds queued mail for your first prompt
instead.) Nothing you can see in a pi-post listing is a dead end:
anything listed can be messaged and reopened.
